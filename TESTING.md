...
- When deploying the front end to Heroku, a bug was encountered. Stating: `Error: digital envelope routines::unsupported`. This was caused by the <5.0 version of webpack clashing with the <17.0 version of Node.js. The quick and easy fix was to add a line to the build script, allowing for legacy libraries. However, a more permanent fix would be to update webpack. Updating webpack caused issues with the create-react-script used. To not spend to much time on mitigating this issue, the git commits working on webpack were reverted, and instead `--openssl-legacy-prodiver` was edited to the start and build scripts within package.json.
